...
void StaticServer::initStatics() {
  ...
  ([this](std::string path) {
    const std::string filePath = this->pathHandler.connect_two_paths(STATIC_CSS_BASE, path);
    return this->fileHandler.get_file_response(filePath);
  });

  CROW_ROUTE(app, "/static/img/<string>")
  ([this](std::string path){
    const std::string filePath = this->pathHandler.connect_two_paths(STATIC_IMG_BASE, path);
    return this->fileHandler.get_file_response(filePath);
  });

  CROW_ROUTE(app, "/static/file/<string>")
  ([this](std::string path){
    const std::string filePath = this->pathHandler.connect_two_paths(STATIC_FILE_BASE, path);
    return this->fileHandler.get_file_response(filePath);
  });

  CROW_ROUTE(app, "/static/js/<string>")
  ([this](std::string path){
    const std::string filePath = this->pathHandler.connect_two_paths(STATIC_JS_BASE, path);
    return this->fileHandler.get_file_response(filePath);
  });
}